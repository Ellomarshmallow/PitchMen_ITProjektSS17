...
import de.pitchMen.client.ShowAllApplicationsOfUser;
...
public class ReportNavigation extends VerticalPanel {
	...
	public void onLoad() {
		...
		Button report4Btn = new Button("Alle Bewerbungen des Nutzers und dazu gehoerenden Ausschreibungen");
		...
		/*
		 * Jeder Button benötigt einen ClickHandler, um auf Interaktionen des
		 * Nutzers entsprechend reagieren zu können.
		 */
	...
	/**
	 * Die geschachtelte Klasse <code>ReportClickHandler</code> implementiert
	 * das von GWT vorgegebene Interface ClickHandler und behandelt alle Fälle,
	 * in denen der Nutzer auf einen der Buttons klickt.
	 * 
	 * @author Simon
	 */
	private class ReportClickHandler implements ClickHandler {
		...
		/**
		 * Die Report-Nummer wird als Instanzvariable deklariert und im
		 * Konstruktor gesetzt. Somit ist über den später verwendeten
		 * Switch-Case eine einfache Handhabung der verschiedenen Fälle
		 * möglich.
		 */
		...
		/**
		 * Die onClick-Methode ist hier eine verzweigende Weitergabe-Methode,
		 * die je nach gewähltem Report-Szenario die entsprechende Methode
		 * ansteuert.
		 */
		@Override
		public void onClick(ClickEvent event) {
			...
			case 2:
				RootPanel.get("content").add(
						new HTML("<h3>Switch Case this.getAllJobPostingsMathcingPartnerProfile() ausgewählt</h3>"));
				reportContent = this.getAllJobPostingsMatchingPartnerProfileOfUser();
				break;
			case 3:
				RootPanel.get("content").add(
						new HTML("<h3>Switch Case this.getApplicationsRelatedToJobPostingOfUser() ausgewaehlt</h3>"));
				reportContent = this.getApplicationsRelatedToJobPostingsOfUser();
				break;
			case 4:
				RootPanel.get("content").add(
						new HTML("<h3>Switch Case this.getAllApplicationsOfUserWithJobPostings() ausgewählt</h3>"));
				reportContent = this.getAllApplicationsOfUserWithJobPostings();
				break;
			case 5:
				RootPanel.get("content")
						.add(new HTML("<h3>Switch Case this.getProjectInterveawings() ausgewählt</h3>"));
				reportContent = this.getProjectInterveawings();
				break;
			case 6:
				RootPanel.get("content")
						.add(new HTML("<h3>Switch Case this.getFanInFanOutAnalysis() ausgewählt</h3>"));
		...
		private HTML getAllApplicationsOfUserWithJobPostings() {
			...
			ClientsideSettings.getReportGenerator().showAllApplicationsOfUser(
					ClientsideSettings.getCurrentUser(), new AsyncCallback<AllApplicationsOfUser>() {

						@Override
						public void onFailure(Throwable caught) {
							ClientsideSettings.getLogger().severe("Upsi, da funktioniert wohl etwas noch nicht");
		...
		private HTML getApplicationsRelatedToJobPostingsOfUser() {
			...
			ClientsideSettings.getReportGenerator().showApplicationsRelatedToJobPostingsOfUser(
					ClientsideSettings.getCurrentUser(), new AsyncCallback<ApplicationsRelatedToJobPostingsOfUser>() {
		...
		// @Override
		// public void onFailure(Throwable caught) {
		// ClientsideSettings.getLogger().severe("Upsi, da
		// funktioniert wohl etwas nicht richtig");
		//
		// }
		//
		// @Override
		...
		// new
		// AsyncCallback<ApplicationsRelatedToJobPostingsOfUser>()
		// {
		//
		// @Override
		// public void onFailure(Throwable caught) {
		// ClientsideSettings.getLogger()
		// .severe("Upsi, da funktioniert wohl etwas nicht
		// richtig");
		//
		// }
		//
		// @Override
		// public void
		// onSuccess(ApplicationsRelatedToJobPostingsOfUser
		// result) {
		// writer.process(result);
		// reportContent = new HTML(writer.getReportText());
		// }
		...
		private HTML getAllJobPostings() {
			...
			ClientsideSettings.getReportGenerator().showAllJobPostings(new AsyncCallback<AllJobPostings>() {
				...
				@Override
				public void onFailure(Throwable caught) {
					ClientsideSettings.getLogger().severe("Upsi, da funktioniert wohl etwas noch nicht richtig");
