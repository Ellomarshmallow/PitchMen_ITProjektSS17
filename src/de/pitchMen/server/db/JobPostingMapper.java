	  	...
	  	try {
    		...
    		ResultSet rs = stmt.executeQuery("SELECT MAX(id) AS maxid FROM jobPosting");
	   		...
	   		if (rs.next()) {
				 ...
				 */
	   			stmt.executeUpdate("INSERT INTO jobPosting (id, title, text)"
	   					 + "VALUES ( " + jobPosting.getId() + ", '" + jobPosting.getTitle() + "' ,'" + jobPosting.getText() + "')");
	    	}
	    }
	...
	*/
    ...
    public void delete(JobPosting jobPosting) throws ClassNotFoundException {
    	...
    	try {
    		...
    		stmt.executeUpdate("DELETE FROM jobPosting " 
    				+ "WHERE id=" + jobPosting.getId());
    	}
	...
	public JobPosting findById(int id) throws ClassNotFoundException {
        ...
        try {
        	Statement stmt = con.createStatement();
        	ResultSet rs = stmt.executeQuery("SELECT id, title, text FROM jobPosting " 
        			+ "WHERE id =" + id);
